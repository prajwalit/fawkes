...
var execute = function (args) {
  if (args.indexOf ("-w") !== -1 || args.indexOf ("watch") !== -1) {
    ...
    console.log ("Use `whiskers init` to create a config file.");
    ...
    console.log ("Look for `whiskers.json` (Default) file to modify config.");
  }
};
  ...
  fs.readdir (path, function (err, files) {
    for (var i = 0; i < files.length; i++) {
      ...
      if (stat && stat.isDirectory()) { // Dir
        ...
        if (f.match (/.html$/)) {
          console.log ("Compiling : " + f.split (opts.root) [1]); // HTML Files
...
var updateFile = function (filePath, opts) {
  if (filePath.match (/\.html$/)) {
    ...
    exec ("handlebars " + filePath + " -f " + output + " --min --root " + opts.root, function (err) {
      if (err) {
        console.log (err);
      } else {
        console.log ("Compiling : " + filePath.split (opts.root) [1]);
      }
    });
  }
};
